...
def load_openml_suite(suite_id, task, suite_name, limit=-1):
    ...
    for task_id in suite.tasks:
        ...
        try:
            ...
            # FIXED: Added float() cast to prevent stringified float crashes
            q = dataset.qualities
            est_cells = int(float(q.get('NumberOfInstances', 0))) * int(float(q.get('NumberOfFeatures', 0)))
...
def make_model(model_name, task, preprocessor, seed):
    # Isolated specifically for Oblique Random Forest
    ...
    if model_name == "obliquert":
        ...
        raise ValueError(f"Model '{model_name}' is not supported in the Oblique-only environment.")
