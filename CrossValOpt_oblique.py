...
def get_param_grid(model_name):
    ...
    """
    if model_name == "obliquert":
        return {
            # Tests Feature Visibility: "sqrt" (Standard) vs 1.0 (Bagging/Regression)
...
if __name__ == "__main__":
    # === CONFIGURATION BLOCK ===
    # Isolated purely to Oblique Random Forest
    SELECTED_MODELS = ["obliquert"]  
    BENCHMARKS = ["OpenML-CC18", "OpenML-297", "AEON-TSC", "AEON-TSER", "OpenML-334", "OpenML-335", "OpenML-336"] # "OpenML-CC18", "OpenML-297", "AEON-TSC", "AEON-TSER", OpenML-334, OpenML-335, OpenML-336
    # ===========================
    ...
    print(f"Starting Multi-Benchmark GridSearch...")
    ...
    for benchmark in BENCHMARKS:
        ...
        # 1. Load Datasets
        # (Filtering by max cells is now safely handled natively inside PreliminaryTestingOpt)
        datasets = []
        ...
        print(f"Selected {len(datasets)} datasets (pre-filtered by {MAX_CELLS_PER_DATASET:,} cap).")
        
        # Iterate over each model requested
        for model_name in SELECTED_MODELS:
            ...
            if df_results.empty:
                print(f"Error: All tasks failed for {model_name.upper()} on {benchmark}. No results to save.")
                continue
            ...
            is_regression = "regression" in [d["task"] for d in datasets[:1]]
            ...
            print("-" * 50)
            print(f"BENCHMARK COMPLETED: {benchmark} | MODEL: {model_name.upper()}")
            ...
            print(f"Saved {model_name.upper()} on {benchmark} cleanly to results directory.\n")
